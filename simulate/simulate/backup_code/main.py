...
# Initialize the sensors in a square shape
sensors_vertical = []
sensors_horizontal = []

# Define the number of sensors in each row and column
num_sensors = 4

# Define the size of the square
square_size = 2

# Calculate the spacing between sensors
spacing = square_size / (num_sensors - 1)

# Initialize the sensors in a square shape
for i in range(num_sensors):
    x = -square_size / 2 + i * spacing
    y = square_size / 4
    sensors_vertical.append(Sensor(ax, x, y, 'ro'))
...
for i in range(num_sensors):
    x = square_size / 4
    y = square_size / 2 - i * spacing
    sensors_horizontal.append(Sensor(ax, x, y, 'bo'))
...
axpos_select = fig.add_axes([0.025, 0.05, 0.15, 0.15])
...
def update_sensor_plot(horizontal_values, vertical_values):
    ...
    ax2.plot([horizontal_values, 0], [0, vertical_values], 'g-')  # Line between the points
    plt.draw()
...
def update(val):
    ...
    update_timer = Timer(0.001, perform_update)
    update_timer.start()
...
def perform_update():
    ...
    ax.text(0.95, 0.95, f'FPS: {fps:.2f}', transform=ax.transAxes, fontsize=12, verticalalignment='top', horizontalalignment='right', bbox=bbox_props)
...
def on_mouse_move(event):
    ...
    if x_val is not None and y_val is not None:
        ...
        for i, sensor in enumerate(sensors_vertical):
            y_value = y_val + i * 0.1
            sensor.set_position(x_val, y_value)  # Offset each sensor vertically, starting from the top
            if curve_type == 'Sine Wave':
                distance = abs(y_value - sine_wave(x_val, shift_slider.val, freq_slider.val, amp_slider.val))
            else:
                distance = abs(np.sqrt(x_val**2 + (y_value)**2) - radius_slider.val)
        ...
        for i, sensor in enumerate(sensors_horizontal):
            x_value = x_val + i * 0.1
            sensor.set_position(x_value, y_val - 0.1)  # Offset each sensor horizontally, starting from the right and shift y by 0.1
            if curve_type == 'Sine Wave':
                distance = abs(y_val - 0.1 - sine_wave(x_value, shift_slider.val, freq_slider.val, amp_slider.val))
            else:
                distance = abs(np.sqrt((x_value)**2 + (y_val - 0.1)**2) - radius_slider.val)
